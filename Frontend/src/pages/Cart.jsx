...
export default function Cart({ cart = {}, onInc, onDec, onRemove, onClearCart }) {
  ...
  useEffect(() => {
    api("/products").then((res) => {
      const map = {};
      (Array.isArray(res) ? res : (res.products || [])).forEach((p) => { map[p.id] = p; });
      setProductMap(map);
    }).catch(() => setProductMap({}))
  ...
  return;
    ...
    if (method === "Cash On Delivery") {
      try{
        await api("/orders/checkout", { method: "POST", body: { address: payload.address, paymentMethod: "COD" }, auth: true });
 ...
 if (method === "Digital Payment") {
  try{
    ...
    const savedOrderId = orderRes.id; // ← this is the DB order ID

    // STEP 2: Create Razorpay payment session
    const rzpOrder = await api(`/payment/create-order?amount=${total}`, { method: "POST", auth: true });
 ...
 async function loadRazorpayAndPay({ key, order, payload, savedOrderId }){
  ...
  body: {
    ...
    orderId: savedOrderId  // you need to save the order ID from the checkout step
  }
});
        ...
        <div className="flex-1 space-y-8 md:space-y-12">
          ...
          <div className="space-y-6">
            {items.map(({ p, qty }) => {
              ...
              return (
                <div key={p.id} className="group relative bg-white rounded-[2.5rem] p-6 md:p-8 shadow-sm border border-gray-100 flex flex-col md:flex-row items-center gap-8 hover:shadow-2xl hover:border-emerald-200 transition-all animate-fade-in group">
                  <div 
                  ...
                  >
                    <img src={fileUrl(p.imageUrl)} alt={p.name} className="w-full h-full object-contain mix-blend-multiply group-hover:scale-110 transition-transform duration-500" />
                  </div>
